//Change in Line no :29,30,36,39,44,45
    ...
    public void minOperations() {
        ...
        for (i = 0; i < n; i++){
            for (j = 0; j <= 7; j++){     //Changed
                ...
                dp[i + 1][j | 4] = Math.min(dp[i + 1][j | 4], dp[i][j] + red.get(i) + green.get(i));    //Changed
            }
        }
        j = 0;
        for (i = 0; i < n; i++){
            if (green.get(i) != 0) j |= 2;  //Changed
            if (red.get(i) != 0) j |= 1;    //Changed
            if (blue.get(i) != 0) j |= 4;
        }
